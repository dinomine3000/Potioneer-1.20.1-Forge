...
import static net.dinomine.potioneer.util.misc.ModNbtUtils.PotionInfoTag.MAX_FLASK_AMOUNT;
import static net.dinomine.potioneer.util.misc.ModNbtUtils.PotionInfoTag.MAX_VIAL_AMOUNT;
...
public class PotionContentData {
    ...
    public boolean isValidContainer(ItemStack stack){
        ...
        if(!ModNbtUtils.hasTag(ModNbtUtils.TAGS.POTION, stack)) return true;

        CompoundTag info = ModNbtUtils.getTagFromItem(ModNbtUtils.TAGS.POTION, stack);
        int level = ModNbtUtils.PotionInfoTag.getPotionAmount(info);
        if(item == ModItems.FLASK.get()){
            return level < MAX_FLASK_AMOUNT && ModNbtUtils.PotionInfoTag.getPotionName(info).equals(name);
        } else {
