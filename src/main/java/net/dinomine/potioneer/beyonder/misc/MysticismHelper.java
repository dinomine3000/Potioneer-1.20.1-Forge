    ...
    public static DivinationResult doDivination(ItemStack item, Player seer, int radius, BlockPos position, int sequenceId){
        ...
        if(item.isEmpty()){
            ...
            if(Math.random() < 0.5f){
                ...
                boolean yesNo = true;
                String clue = Beyonder.getSequenceNameFromId(sequenceId - 1, true);
        ...
        if(item.is(ModItems.BEYONDER_POTION.get())){
            if(item.hasTag() && item.getTag().contains("potion_info")){
                ...
                if(name.equalsIgnoreCase("conflict")){
                    ...
                    try{
                        ...
                        clue = Beyonder.getPathwayName(potionSequence, true);
            ...
            Optional<EntityBeyonderManager> cap = seer.getCapability(BeyonderStatsProvider.BEYONDER_STATS).resolve();
            if(cap.isPresent()){
        ...
        if(!positions.isEmpty()){
            ...
            clue = Beyonder.getPathwayName(highestSequence, true);
        }
