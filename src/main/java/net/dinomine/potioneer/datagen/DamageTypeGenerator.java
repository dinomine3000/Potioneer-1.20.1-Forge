...
public class DamageTypeGenerator extends DamageTypeTagsProvider {
    ...
    protected void addTags(HolderLookup.Provider pProvider) {
        tag(DamageTypeTags.BYPASSES_ARMOR).add(LOW_SANITY, ASTEROID, CHRYON_PIERCE, CRIT, ARREST, STRIKE, EXECUTION);
        tag(DamageTypeTags.BYPASSES_SHIELD).add(LOW_SANITY, ASTEROID, CHRYON_PIERCE, CRIT, STRIKE, EXECUTION);
...
//        tag(DamageTypeTags.AVOIDS_GUARDIAN_THORNS).add();
        ...
        tag(DamageTypeTags.BYPASSES_INVULNERABILITY).add(LOW_SANITY, LOW_SANITY_KILL, STRIKE, EXECUTION);

        tag(Tags.ABSOLUTE).add(ARREST, CRIT, LOW_SANITY, LOW_SANITY_KILL, LOW_SPIRITUALITY, EXILE, TYRANT_HEALING, STRIKE, EXECUTION);
